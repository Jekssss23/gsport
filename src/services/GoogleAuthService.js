...
const getNativeGoogleSignin = () => {
  try {
    ...
    return mod?.GoogleSignin || mod;
  } catch (e) {
    console.warn('Native GoogleSignin module disabled/unavailable:', e.message);
    return null;
  }
};
...
const promptNativeGoogleAuth = async () => {
  try {
    ...
    if (!GoogleSignin || typeof GoogleSignin.configure !== 'function') {
      console.warn('Native GoogleSignin is not available on this device');
      return { type: 'error', error: 'Google Sign-In is not supported on this device.' };
    }
