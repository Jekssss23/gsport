              ...
              if ($q_cek_password>0) {
                ...
                if (count($hak_akses)>0) {
                    ...
                    if (empty($firebase_uid)) {
                        $this->db->trans_rollback();
                        log_message('error', 'User: Firebase UID is empty for user: ' . $email);
                    ...
                    $firestore_data = [
                        'email' => $email,
                        'name' => $nama,
                        'role' => 'admin', // Default to admin role for staff created via admin panel
                    ...
                    if (!$fs_result) {
                        $this->db->trans_rollback();
                        log_message('error', 'User: Failed to create Firestore document for UID: ' . $firebase_uid);
                    ...
                    // Also save to employees table for Big Data
                    ...
                    $employee_data = [
                        ...
                        'alamat' => $alamat,
                        'jabatan' => $jabatan,
                        'foto' => isset($new_file_name) && $nama_file != '' ? $new_file_name : null,
                        'firebase_uid' => $firebase_uid,
                        'app_username' => $username,
                        'has_app_login' => 1,
                        'status' => 'active',
                    ];
                    // Use the same id_user as id for employees
                    $this->employee_m->save($employee_data, $id_user);
        ...
        if ( ! $this->upload->do_upload('berkas')){
            ...
            if ($error>0) {
                $redirect = 'user/admin/user';
                 ...
                 // Also sync to employees table for Big Data
                 $this->load->model('Employee_model', 'employee_m');
                 $employee_data = [
                     'nama' => $nama,
                     'email' => $email,
                 ...
                 if (isset($data_insert['foto'])) {
                     $employee_data['foto'] = $data_insert['foto'];
                 }
                 $this->employee_m->save($employee_data, $id_user);
