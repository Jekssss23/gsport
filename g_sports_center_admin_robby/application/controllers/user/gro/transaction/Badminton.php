...
class Badminton extends CI_Controller {
    ...
    public function jadwal_tersedia()
    {
        ...
        $q_shoultcook = $this->db->query("SELECT selisih_tanpa_shoutlecook  from badminton_shoutlecook")->row_array();
        ...
        for ($i=7; $i < 24 ; $i++) { 
            ...
            if (in_array($jam, $kumpul_jam_terpakai)) {
                $data = [
                    'jam'=>$jam,
                    'selisih_tanpa_shoutlecook'=>$q_shoultcook['selisih_tanpa_shoutlecook'] =='' ? 0 : $q_shoultcook['selisih_tanpa_shoutlecook'],
                    'harga'=>$harga,
                    'tersedia'=>0,
                ];
            }else{
 ...
 public function simpanedit_identitas(){
        ...
        $this->db->trans_begin();
            ...
            $where = [
                'id_booking_badminton' => $id_pengunjung,
              
            ];


     
        $this->db->update('identitas_order_badminton', $data , $where );
 ...
 public function simpan_transaksi_badminton(){
        ...
        $shoutlecoock = $this->input->post('shoutlecoock');
        ...
        if ($this->input->post('metode_pembayaran_2')!=null) {
            ...
            $data_pembayaran = [
                [
                    ...
                    'item_transaksi' => 'Badminton - Harian', 
                    'keterangan'=>'Pembayaran Badminton - Harian selama '.$jumlah_jam_main.' Jam pada tanggal '.$tgl_main,
            ...
            $data_pembayaran = [
                    ...
                    'item_transaksi' => 'Badminton - Harian', 
                    'keterangan'=>'Pembayaran Badminton - Harian selama '.$jumlah_jam_main.' Jam',
                    'tgl_transaksi' => timestamp(), 
                    'id_user' => id_user(), 
                    'status' => 'Preview',  
            ];
 ...
 public function print_transaksi_badminton_harian($id_transaksi)
    {
...
jam_main 
        ...
        $data['t']='';
        $html = $this->load->view('user/gro/transaction/print/badminton_harian', $data, true);
    ...
    public function dt_member()
    {
        $data = [];
         ...
         // untuk order by
         if ($key) {
            $q = $this->db->query("SELECT  nama, no_hp,alamat,keterangan,id_booking_badminton from identitas_order_badminton where kategori_order='New' and (nama like'%$key%' ) limit $start, $length")->result_array();
         }else{
            $q = $this->db->query("SELECT  nama, no_hp,alamat,keterangan,id_booking_badminton from identitas_order_badminton where kategori_order='New'  $order_by limit $start, $length ")->result_array();
         }
         $all_data = $this->db->query("SELECT  id_booking_badminton from identitas_order_badminton  where kategori_order='New' ")->num_rows();
        ...
        foreach ($q as $k => $v) {
            ...
            $row[]  = '
            <div class="btn-group">
            <a href="javascript:void(0)" onclick="pilih_member('."'".$v['nama']."'".','."'".$v['alamat']."'".','."'".$v['no_hp']."'".','."'".$v['id_booking_badminton']."'".')" class="btn btn-outline-info btn-xs"><i class="fa fa-check"></i></a>
            </div>';
