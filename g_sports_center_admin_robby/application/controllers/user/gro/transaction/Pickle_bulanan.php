...
class Pickle_bulanan extends CI_Controller {
    ...
    public function jadwal_tersedia()
    {
        ...
        $q_master = $this->db->query("SELECT jam_mulai, jam_berakhir from master_pickle")->result_array();
    ...
    public function jadwal_pickle_tersetting()
    {
        $tgl = $this->input->post('tgl_main');
        $lapangan = $this->input->post('lapangan');
        $q = $this->db->query("SELECT biaya from jadwal_badminton where status='Set'")->result_array();
    ...
    public function harga()
    {
        ...
        $q_biaya = $this->db->query("SELECT harga from master_pickle")->result_array();
        $harga = [
            'siang'=> [
 ...
 public function update_jadwal_main(){
        ...
        $where = ['id_detail_booking_badminton'=>$id_jadwal];
        $this->db->trans_begin();
        $data = [
            'jam_main'=>$jam_main, 
            'tgl_main'=>$tgl_main, 
        ...
        $this->db->update('jadwal_badminton', $data, $where);
 ...
 public function set_jadwal_main(){
        ...
        $delete = $this->db->query("DELETE FROM jadwal_badminton where status='Set' and tgl_main='$tgl_main' and jadwal_ke='$jadwal_ke'");
        ...
        foreach ($jam_main as $k => $v) {   
            ...
            $data = [
                // 'id_identitas_order_pickle' => $id_identitas_order_pickle,
                'lapangan' => $lapangan,
                'tgl_main' => $tgl_main,
                'jam_main' => $jam,
                'biaya' => $biaya,
                'jadwal_ke' => $jadwal_ke,
                'fasilitas'=>'Pickle - Member Bulanan',
                'status' => 'Set',
            ];
 ...
 public function simpan_transaksi_pickle_bulanan(){
        ...
        $diskon_pickle_bulanan   = $this->input->post('diskon_bulanan');
        ...
        $data_transaksi = [
            ...
            'jam_transaksi' => jams(),
            'order_ke' => $next_order,
            'order_ke_fasilitas' => next_order_fasilitas('Pickle - Member Bulanan'),
            'total' => $simpan_total,
        ...
        $kumpul_pendapatan = [];
                ...
                $this->db->insert('identitas_order_pickle', $data_identitas_order);
        $id_identitas_order_pickle = $this->db->insert_id();
              $q = $this->db->update('jadwal_badminton',['status'=>$status,'id_transaksi'=>$id_transaksi,'id_identitas_order_pickle'=>$id_identitas_order_pickle], ['status'=>'Set']);
  
        $q_data = $this->db->select('jadwal_ke, lapangan, tgl_main,jam_main,biaya,status, jadwal_ke ')->where(['id_transaksi'=>$id_transaksi])->get('jadwal_badminton')->result_array();
        ...
        foreach ($q_data as $k => $v) {   
            ...
            $biaya_setelah_diskon = $biaya - $diskon_bulanan;
             $pendapatan = [
                ...
                'keterangan'=> 'Pickle - Member Bulanan '.$nama.'<br>Jadwal ke : '.$jadwal_ke.' <br>tanggal '.$tgl.'<br>jam '.$jam,
        ...
        if ($this->input->post('metode_pembayaran_2')!=null) {
            ...
            $data_pembayaran = [
                [
                    ...
                    'item_transaksi' => 'Pickle - Member Bulanan', 
                    'keterangan'=>'Pembayaran Pickle - Member Bulanan a.n. '.$nama,
 ...
 public function print_transaksi_pickle_bulanan($id_transaksi)
    {
        ...
        ")->row_array();
         ...
         from transaksi t 
         left join identitas_order_pickle idf on t.id_transaksi = idf.id_transaksi
...
jam_main, tgl_main 
        ...
        $q_diskon = $this->db->query("SELECT diskon from diskon_member_pickle")->row_array();
        ...
        $data['t']='';
        $html = $this->load->view('user/gro/transaction/print/pickle_bulanan', $data, true);
