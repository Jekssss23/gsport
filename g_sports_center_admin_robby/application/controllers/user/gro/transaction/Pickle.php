...
class pickle extends CI_Controller {
    ...
    public function jadwal_tersedia()
    {
        ...
        if ($hari >= 6) {
            ...
            $jam_mulai = explode(':', $q_master[2]['jam_mulai']);
            $jam_selesai = explode(':', $q_master[2]['jam_berakhir']);
 ...
 public function simpanedit_identitas(){
        ...
        $this->db->trans_begin();
            ...
            $where = [
                'id_booking_pickle' => $id_pengunjung,
              
            ];


     
        $this->db->update('identitas_order_pickle', $data , $where );
 ...
 public function simpan_transaksi_pickle(){
        ...
        if ($this->input->post('metode_pembayaran_2')!=null) {
            ...
            $data_pembayaran = [
                [
                    ...
                    'item_transaksi' => 'pickle - Harian', 
                    'keterangan'=>'Pembayaran pickle - Harian selama '.$jumlah_jam_main.' Jam pada tanggal '.$tgl_main,
            ...
            $data_pembayaran = [
                    ...
                    'item_transaksi' => 'pickle - Harian', 
                    'keterangan'=>'Pembayaran pickle - Harian selama '.$jumlah_jam_main.' Jam',
                    'tgl_transaksi' => timestamp(), 
                    'id_user' => id_user(), 
                    'status' => 'Preview',  
            ];
 ...
 public function print_transaksi_pickle_harian($id_transaksi)
    {
...
jam_main 
        ...
        $data['t']='';
        $html = $this->load->view('user/gro/transaction/print/pickle_harian', $data, true);
    ...
    public function dt_member()
    {
        $data = [];
         ...
         // untuk order by
         if ($key) {
            $q = $this->db->query("SELECT  nama, no_hp,alamat,keterangan,id_booking_pickle from identitas_order_pickle where kategori_order='New' and (nama like'%$key%' ) limit $start, $length")->result_array();
         }else{
            $q = $this->db->query("SELECT  nama, no_hp,alamat,keterangan,id_booking_pickle from identitas_order_pickle where kategori_order='New'  $order_by limit $start, $length ")->result_array();
         }
         $all_data = $this->db->query("SELECT  id_booking_pickle from identitas_order_pickle  where kategori_order='New' ")->num_rows();
        ...
        foreach ($q as $k => $v) {
            ...
            $row[]  = '
            <div class="btn-group">
            <a href="javascript:void(0)" onclick="pilih_member('."'".$v['nama']."'".','."'".$v['alamat']."'".','."'".$v['no_hp']."'".','."'".$v['id_booking_pickle']."'".')" class="btn btn-outline-info btn-xs"><i class="fa fa-check"></i></a>
            </div>';
