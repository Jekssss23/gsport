                                              ...
                                              <li class="list-group-item">
                                                <div class="row">
                                                  <div class="col-md-12">
                                                    ...
                                                    <div class="form-group" id="form_tgl_main">
                                                          <label>Shouttlecook</label>
                                                          <select class="form-control" name="shoutlecoock" id="shoutlecoock">
                                                            <option value="1">Pakai Shuttlecock </option>
                                                            <option value="0">Tanpa Shuttlecock </option>
                                                            <!-- <option>Sudah Main</option> -->
                                                           </select>
                                                        </div>
                                                          ...
                                                          <div class="widget-content p-0 w-100">
                                                              <div class="widget-content-outer">
                                                                  ...
                                                                  <div class="widget-content-wrapper">
                                                                      ...
                                                                      <div class="text-muted opacity-6" id="keterangan_biaya"></div>
                                                              </div>
                                                          </div>
                                                      </div>
                                                  </div>
  ...
  <script type="text/javascript">
    ...
    function konfirmasi(){
            ...
            Swal.fire('Error','Transaksi tidak bisa dilanjutkan. minimal pembayaraan 50% ('+number_format(minimal_dp)+')','error');
             ...
             if (kategori_diskon=='Student Card') {
              ...
              if (id_student_card=='') {
                ...
                $('#konfirmasi_badminton').modal('show');
              }
            }else{
              $('#konfirmasi_badminton').modal('show');
        ...
        if (simpan_kembalian<0) {
          $('#konfirmasi_badminton_caption_dibayar').html('DP');
          $('#konfirmasi_badminton_caption_kembalian').html('Sisa Pembayaran');
        }else{
          $('#konfirmasi_badminton_caption_dibayar').html('Dibayar');
          $('#konfirmasi_badminton_caption_kembalian').html('Kembalian');
    ...
    $('#konfirmasi_badminton').find('#konfirmasi_badminton_biaya').html(number_format(simpan_total));
    $('#konfirmasi_badminton').find('#konfirmasi_badminton_potongan').html(diskon);
    // $('#konfirmasi_badminton').find('#konfirmasi_badminton_student_card').html(formdata[25].value);
    $('#konfirmasi_badminton').find('#konfirmasi_badminton_tagihan').html(number_format(simpan_tagihan));
    $('#konfirmasi_badminton').find('#konfirmasi_badminton_dibayar').html(number_format(simpan_dibayar));
    $('#konfirmasi_badminton').find('#konfirmasi_badminton_kembalian').html(number_format(simpan_kembalian));





        $('#konfirmasi_badminton').find('#konfirmasi_badminton_tgl_main').html(tgl_main + '<br>Lapangan '+ lapangan);
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_jam_main').html(`<ol>`);
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_nama').html(nama);
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_alamat').html(alamat);
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_nohp').html(nohp);
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_keterangan').html(keterangan);
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_status').html(status);
      $('input[type=checkbox]').each(function () {
        ...
        $('#konfirmasi_badminton').find('#konfirmasi_badminton_jam_main').append(`</ol>`);
    ...
    function tampilkan_jadwal_main(){
     ...
     $.ajax(
            {
              url     : baseUrl('/user/gro/transaction/badminton/jadwal_tersedia'),
              type    : 'POST',
              dataType : 'JSON',
              data    : { 
                tgl_main : tgl_main,
                ...
                $.each(data, function (k,v){
                  ...
                  $.each(v, function(k_jam, v_jam){
                     if (v_jam.tersedia=='0') {
                      ...
                      $('.show_jam_' +k ).append('<input type="checkbox" name="jam_main[]" class="checkbok_jam" value="'+v_jam.jam+'|'+v_jam.harga+'" style="margin-right:5px" onclick="hitung_jam(this, '+v_jam.selisih_tanpa_shoutlecook+')" biaya="'+v_jam.harga+'" jam_main="'+v_jam.jam+'"><span style="color:green;">'+v_jam.jam+'</span><br>');
    ...
    var kumpul_jam = [];
    function hitung_jam(x, selisih_tanpa_shoutlecook){

        var cek_soutlecoock = $('#shoutlecoock').val();
        $('#potongan_harga').val('').change();
      jumlah_jam = $('.checkbok_jam').filter(':checked').length;
      if (cek_soutlecoock==0) {
          var biaya = $(x).attr('biaya') - selisih_tanpa_shoutlecook;
          var keterangan_biaya = "Harga Awal : " + number_format($(x).attr('biaya')) + '<br>Tanpa shoutlecook : diskon ' + number_format(selisih_tanpa_shoutlecook);
      ...
      if (jumlah_jam==0) {
        ...
        $('#keterangan_biaya').html(keterangan_biaya);
...
$('#input_dibayar').keyup(function(){
  ...
  function simpan_transaksi_badminton(){
    ...
    var formdata = $('#form_badminton').serialize();
     $.ajax(
            {
              url     : baseUrl('/user/gro/transaction/badminton/simpan_transaksi_badminton'),
              type    : 'POST',
              // dataType : 'JSON',
...
<div class="alert alert-info">Transaksi badminton - Harian Disimpan <br>
...
<button id="btn_tab_new_transaction" class="btn btn-info" data-toggle="tab" href="#new_transaction" onclick="show_pilihan_fasilitas()">
  ...
  function print_transaksi(id_transaksi){
     $('#konfirmasi_badminton').modal('hide');
    $('#print_transaksi_gym').modal('show');
    $('#print_transaksi_gym').find('#struk_order_member_gym').html(`
      <iframe src="`+ baseUrl('/user/gro/transaction/badminton/print_transaksi_badminton_harian/') + id_transaksi +`/?status_print=settlement&action=Lunas" width="100%" height="400px"></iframe>
      `);
  }
