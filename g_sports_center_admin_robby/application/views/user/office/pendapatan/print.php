...
if ($id_kasir=='Semua') {
  ...
  $where_user = "and t.id_user='$id_kasir'";
  $user = $this->db->query("SELECT id_user, nama from master_user where id_user='$id_kasir'")->row_array();
  $kasir = "Kasir : ".$user['nama'];

}

 ?>
<p style="text-align:center"><?php echo $judul ?><br> <?php echo $kasir ?></p>
<?php   
...
foreach ($akun_pendapatan as $k => $v) {  
  ...
  <tr>
    ...
    if ($filter=='bulanan') {
      if ($id_akun_pendapatan==10) {
        $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan = '$id_akun_pendapatan' and (month(tgl_transaksi) ='$bulan' and year(tgl_transaksi) ='$tahun') and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user")->result_array();
      }
      elseif ($id_akun_pendapatan==3 || $id_akun_pendapatan==4 || $id_akun_pendapatan==9) {
        $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan like '%$id_akun_pendapatan%' and (month(tgl_transaksi) ='$bulan' and year(tgl_transaksi) ='$tahun') and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user")->result_array();
      }else{
        $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan = '$id_akun_pendapatan' and (month(tgl_transaksi) ='$bulan' and year(tgl_transaksi) ='$tahun') and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user")->result_array();
      }
    ...
    elseif ($filter=='tahunan') {
      ...
      $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan like '%$id_akun_pendapatan%' and (year(tgl_transaksi) ='$tahun') and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user ")->result_array();
    ...
    $jam_akhir = '23:59:59';
      ...
      $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan like '%$id_akun_pendapatan%' and  ( t.tgl_transaksi BETWEEN '$tgl_awal $jam_mulai' and '$tgl_akhir $jam_akhir') and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user ")->result_array();



    }else{
      if ($id_akun_pendapatan==10) {
        $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_transaksi_pelunasan, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan = '$id_akun_pendapatan' and t.tgl_transaksi like '%$tgl%' and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user ")->result_array();
        # code...
      }
      elseif ($id_akun_pendapatan==3 || $id_akun_pendapatan==4 || $id_akun_pendapatan==9) {
        $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_transaksi_pelunasan, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan like '%$id_akun_pendapatan%' and t.tgl_transaksi like '%$tgl%' and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user ")->result_array();
        # code...
      }else{
        $q_transaksi = $this->db->query("SELECT t.id_transaksi, t.id_transaksi_pelunasan, t.id_akun_pendapatan, t.pembayaran, t.fasilitas, t.tagihan from transaksi t where t.id_akun_pendapatan = '$id_akun_pendapatan' and t.tgl_transaksi like '%$tgl%' and t.status!='Preview' and t.id_metode_pembayaran like '%$id_metode_pembayaran%' $where_user ")->result_array();
      ...
      foreach ($q_transaksi as $k_t => $v_t) {
        ...
        if ($v_t['pembayaran']=='1 Metode Pembayaran') {
          if ($v_t['fasilitas']=='Gym - Special Membership') {
            ...
            if ($id_akun_pendapatan == 2) {
          ...
          else if($v_t['fasilitas']=='Pickle - Turnamen' ){

              $hitung_pemasukan_pickle = 0;
              $hitung_pengeluaran_pickle = 0;
              $hitung_pemasukan_lainlain = 0;
              $hitung_pengeluaran_lainlain = 0;
              ...
              foreach ($q_pendapatan as $k_pd => $v_pd) {
                if ($v_pd['kategori']=='Masuk') {
                  if ($v_pd['id_akun_pendapatan']==10) {
                    $hitung_pemasukan_pickle +=$v_pd['nilai'];
                  }else{
                    $hitung_pemasukan_lainlain +=$v_pd['nilai'];
                  }
                }else{
                  if ($v_pd['id_akun_pendapatan']==10) {
                    $hitung_pengeluaran_pickle +=$v_pd['nilai'];
                  }else{
                    $hitung_pengeluaran_lainlain +=$v_pd['nilai'];
                  }
                }
              }
            $hasil_pickle = $hitung_pemasukan_pickle - $hitung_pengeluaran_pickle  ;
            $hasil_lainlain = $hitung_pemasukan_lainlain - $hitung_pengeluaran_lainlain  ;
            if ($id_akun_pendapatan == 10) {
              // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $hasil_swim);
              $hitung_nilai_pemasukan += $hasil_pickle;
            }else{
              // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $hasil_gym);
              $hitung_nilai_pemasukan += $hasil_lainlain ;
            }
          ...
          if ($v_t['fasilitas']=='Gym - Special Membership') {
            ...
            $id_transaksi_pelunasan = $v_t['id_transaksi_pelunasan'];
              $q_pendapatan = $this->db->query("SELECT p.nilai, p.kategori, p.id_akun_pendapatan  from pendapatan p  where p.id_transaksi='$id_transaksi'")->result_array();
              $hitung_pemasukan_futsal = 0;
              $hitung_pengeluaran_futsal = 0;
              $hitung_pemasukan_lainlain = 0;
            ...
            foreach ($q_pembayaran as $k_pb => $v_pb) {
              if ($v_pb['id_metode_pembayaran']==$id_metode_pembayaran) {
                  if ($id_akun_pendapatan == 1) {
                    if ($k_pb==0) {
                      if ($v_pb['pendapatan'] == $v_pb['dibayar']) {
                        if ($hasil_futsal==$v_pb['pendapatan']) {
                          $hitung_nilai_pemasukan += $hasil_futsal;
                          // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $hasil_futsal);
                      ...
                      if ($v_pb['pendapatan'] == $v_pb['dibayar']) {
                        ...
                        elseif ($hasil_futsal > $v_pb['pendapatan']) {
                            $disetor = $v_pb['pendapatan'];
                            $hitung_nilai_pemasukan += $disetor;
                            // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $disetor);
                        } else  {
                        ...
                        $pendapatan_pertama = $q_pembayaran[0]['pendapatan'];
                        if ($pendapatan_pertama > $hasil_futsal) {
                            $disetor = 0;
                            $hitung_nilai_pemasukan += $disetor;
                            // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $disetor);
                        }else{
                            $setelah_pendapatan_pertama = $hasil_futsal - $pendapatan_pertama;
                            // $disetor = $v_pb['pendapatan'];
                            $hitung_nilai_pemasukan += $setelah_pendapatan_pertama;
                            // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $setelah_pendapatan_pertama);
                        }
                    ...
                    if ($k_pb==0) {
                      if ($v_pb['pendapatan'] == $v_pb['dibayar']) {
                        if ($hasil_lainlain==$v_pb['pendapatan']) {
                          $hitung_nilai_pemasukan += $hasil_lainlain;
                            // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $hasil_lainlain);
                        }else{
                          if ($hasil_lainlain > $v_pb['pendapatan']) {
                              $disetor = $v_pb['pendapatan'];
                              $hitung_nilai_pemasukan += $disetor;
                          } else  {
                              $disetor = $hasil_lainlain;
                              $hitung_nilai_pemasukan += $disetor;
                          }
                            // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $disetor);
                        }
                      ...
                      if ($v_pb['pendapatan'] == $v_pb['dibayar']) {
                        ...
                        elseif ($hasil_lainlain > $v_pb['pendapatan']) {
                            $disetor = $v_pb['pendapatan'];
                            $hitung_nilai_pemasukan += $disetor;
                        } else  {
                            $disetor = $hasil_lainlain;
                        ...
                        $pendapatan_pertama = $q_pembayaran[0]['pendapatan'];
                        if ($pendapatan_pertama > $hasil_lainlain) {
                            $disetor = 0;
                            $hitung_nilai_pemasukan += $disetor;
                        }else{
                            $setelah_pendapatan_pertama = $hasil_lainlain - $pendapatan_pertama;
                            $disetor = $v_pb['pendapatan'];
                            $hitung_nilai_pemasukan += $setelah_pendapatan_pertama;
                            // array_push($kumpul_total_pemasukan_per_metode[$index]['nilai'], $setelah_pendapatan_pertama);
                        }
